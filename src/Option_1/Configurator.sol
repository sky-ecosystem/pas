...
interface ATWLStateLike {
    function defaults(bytes32) external view returns (uint256, uint256);
    function rateLimits(bytes32) external view returns (RateLimitsLike);
}
...
contract Configurator {
    ...
    function initRateLimit(bytes32 prime, bytes32 key) external toll {
        (uint256 maxAmount, uint256 slope) = atwlState.defaults(key);
        require(maxAmount > 0 && slope > 0, "Configurator/wrong-defaults");
        RateLimitsLike.RateLimitData memory current = atwlState.rateLimits(prime).getRateLimitData(key);
        require(current.maxAmount == 0 || current.slope == 0, "Configurator/already-init");

        atwlState.rateLimits(prime).setRateLimitData(key, maxAmount, slope);

        zzz[prime][key] = block.timestamp;

        emit SetRateLimit(prime, key, maxAmount, slope, true, block.timestamp);
    }
