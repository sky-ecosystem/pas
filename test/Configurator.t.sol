...
// Mock PAU contract implementing RateLimits interface
contract MockPau {
    ...
    function controllerFunction(uint256 value) external returns (uint256) {
        ...
        require(!shouldFail, "MockPau/controller-function-failed");
        return value * 2;
    }

    fallback() external {
        lastCallData = msg.data;
        require(!shouldFail, "MockPau/fallback-failed");
    }
}
...
contract ConfiguratorTest is DssTest {
    ...
    function _setupCBeam(address pau, address cBeam) internal {
        beamState.addCBeam(cBeam);
        beamState.setCBeamForPau(pau, cBeam);
    }
    ...
    function _setupRateLimitData(
        ...
        pau.setRateLimitData(key, maxAmount, slope, lastAmount, lastUpdated);
        pau.setCurrentRateLimit(lastAmount);
    }

    function _setupDefaultRateLimits(bytes32 key, address pau, uint256 maxAmount, uint256 slope) internal {
    ...
    function testAuthNotAuthorizedCBeam() public {
        ...
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/not-authorized-cBeam");
        configurator.setRateLimit(address(pau1), key, 1_000 * WAD, 10 * WAD);
    }

    function testAuthWithAuthorizedCBeam() public {
        bytes32 key = keccak256("test-key");
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 500 * WAD, 5 * WAD, 500 * WAD, block.timestamp);

        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 400 * WAD, 4 * WAD);

        RateLimitsLike.RateLimitData memory data = pau1.getRateLimitData(key);
        assertEq(data.maxAmount, 400 * WAD, "maxAmount should be updated by authorized cBeam");
    }

    function testAuthCBeamCannotConfigureDifferentPau() public {
        bytes32 key = keccak256("test-key");
        _setupCBeam(address(pau1), CBEAM1);

        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/not-authorized-cBeam");
        configurator.setRateLimit(address(pau2), key, 1_000 * WAD, 10 * WAD);
    }
    ...
    function testSetRateLimitDecreasing() public {
        ...
        vm.prank(CBEAM1);
        vm.expectEmit();
        emit SetRateLimit(address(pau1), key, 400 * WAD, 4 * WAD);
        configurator.setRateLimit(address(pau1), key, 400 * WAD, 4 * WAD);
    ...
    function testSetRateLimitWithinDefaults() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 800 * WAD, 8 * WAD);
    ...
    function testSetRateLimitDecreasingOneParameter() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 400 * WAD, 5 * WAD);
    ...
    function testSetRateLimitIncreasingTooSoon() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_200 * WAD, 12 * WAD);
        _setupRateLimitData(pau1, key, 1_000 * WAD, 10 * WAD, 1_000 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 3_600); // 1 hour hop
        beamState.setMaxChange(address(pau1), 15 * WAD / 10); // 1.5x

        vm.warp(block.timestamp + 3_600); // Warp to allow first increase

        // First increase beyond defaults (unsafe, respects maxChange)
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_300 * WAD, 13 * WAD); // 1.3x from current

        // Try to increase again immediately (should fail)
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/increment-too-soon");
        configurator.setRateLimit(address(pau1), key, 1_400 * WAD, 14 * WAD);
    }

    function testSetRateLimitIncreasingAfterHop() public {
        bytes32 key = keccak256("after-hop-key");
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 3_000 * WAD, 30 * WAD);
        _setupRateLimitData(pau1, key, 2_000 * WAD, 20 * WAD, 2_000 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 3_600); // 1 hour hop
        beamState.setMaxChange(address(pau1), 15 * WAD / 10); // 1.5x
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 3_000 * WAD, 25 * WAD); // Exactly at defaults, 1.5x maxAmount, 1.25x slope
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 4_500 * WAD, 37 * WAD); // Exactly 1.5x from 3_000, ~1.48x slope
    ...
    function testSetRateLimitMaxChangeEnforcedMaxAmount() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 800 * WAD, 8 * WAD, 800 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 1); // Minimal hop delay
        beamState.setMaxChange(address(pau1), 15 * WAD / 10); // 1.5x
        ...
        configurator.setRateLimit(address(pau1), key, 1_300 * WAD, 8 * WAD); // 1.625x increase (> 1.5x)
    }

    function testSetRateLimitMaxChangeEnforcedSlope() public {
        bytes32 key = keccak256("maxchange-slope-key");
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_0000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 1_000 * WAD, 8 * WAD, 1_000 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 1); // Minimal hop delay
        beamState.setMaxChange(address(pau1), 15 * WAD / 10); // 1.5x
        ...
        configurator.setRateLimit(address(pau1), key, 1_000 * WAD, 13 * WAD); // 1.625x increase (> 1.5x)
    }

    function testSetRateLimitMaxChangeAtLimit() public {
        bytes32 key = keccak256("maxchange-exact-key");
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_0000 * WAD, 100 * WAD);
        _setupRateLimitData(pau1, key, 1_000 * WAD, 10 * WAD, 1_000 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 3_600);
        beamState.setMaxChange(address(pau1), 15 * WAD / 10); // 1.5x
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_500 * WAD, 15 * WAD);
    ...
    function testZzzTimestampSetOnIncrease() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_200 * WAD, 12 * WAD);

        assertEq(configurator.zzz(address(pau1), key), timeBefore, "zzz should be set to current timestamp");
    }
    ...
    function testZzzTimestampNotSetOnDecrease() public {
        ...
        assertEq(configurator.zzz(address(pau1), key), 0, "zzz should not be set on decrease");
    }
    ...
    function testZzzTimestampUpdatedOnSecondIncrease() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_100 * WAD, 11 * WAD);
        assertEq(configurator.zzz(address(pau1), key), firstTime, "zzz should be set to first increase time");
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_200 * WAD, 12 * WAD);
        assertEq(configurator.zzz(address(pau1), key), secondTime, "zzz should be updated to second increase time");
    }
    ...
    function testSetUnlimitedRateLimit() public {
        ...
        vm.prank(CBEAM1);
        vm.expectEmit();
        emit SetRateLimit(address(pau1), key, type(uint256).max, 0);
        configurator.setRateLimit(address(pau1), key, type(uint256).max, 0);
    ...
    function testLastAmountCappedAtMaxAmount() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 2_000 * WAD, 20 * WAD, 1_500 * WAD, block.timestamp);
        pau1.setCurrentRateLimit(1_500 * WAD);
    ...
    function testLastAmountPreservedWhenBelowMax() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 2_000 * WAD, 20 * WAD);
        _setupRateLimitData(pau1, key, 1_000 * WAD, 10 * WAD, 500 * WAD, block.timestamp);
        pau1.setCurrentRateLimit(500 * WAD);
    ...
    function testGlobalDefaultsUsedWhenNoSpecific() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 900 * WAD, 9 * WAD);
    ...
    function testSpecificDefaultsOverrideGlobal() public {
        ...
        _setupDefaultRateLimits(key, address(pau1), 2_000 * WAD, 20 * WAD); // Specific
        _setupRateLimitData(pau1, key, 500 * WAD, 5 * WAD, 500 * WAD, block.timestamp);

        // Should use PAU1 specific defaults (higher limits)
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_500 * WAD, 15 * WAD);
    ...
    function testGlobalHopUsedWhenNoSpecific() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 900 * WAD, 9 * WAD, 900 * WAD, block.timestamp);

        // Only global hop set (in setUp)
        assertEq(beamState.hop(address(pau1)), 0, "PAU1 specific hop should be 0");
        assertEq(beamState.getHop(address(pau1)), 86_400, "should fallback to global hop");
    ...
    function testGlobalMaxChangeUsedWhenNoSpecific() public {
        ...
        assertEq(beamState.maxChange(address(pau1)), 0, "PAU1 specific maxChange should be 0");
        assertEq(beamState.getMaxChange(address(pau1)), 15 * WAD / 10, "should fallback to global maxChange");
        ...
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/maxChange-maxAmount");
        configurator.setRateLimit(address(pau1), key, 1_500 * WAD, 9 * WAD);
    }
    ...
    function testCallControllerAction() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        beamState.addInitControllerActions(data, address(pau1));

        vm.prank(CBEAM1);
        bytes memory ret = configurator.callControllerAction(address(pau1), data);

        uint256 result = abi.decode(ret, (uint256));
        assertEq(result, 84, "controller function should return 42 * 2");
        assertEq(pau1.lastCallData(), data, "correct data should be passed to PAU");
    }
    ...
    function testCallControllerActionNotWhitelisted() public {
        ...
        configurator.callControllerAction(address(pau1), data);
    }
    ...
    function testCallControllerActionGlobalWhitelist() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupCBeam(address(pau2), CBEAM1);
        beamState.addInitControllerActions(data, address(0)); // Global whitelist

        // Should work for any PAU
        vm.prank(CBEAM1);
        bytes memory ret1 = configurator.callControllerAction(address(pau1), data);
        assertEq(abi.decode(ret1, (uint256)), 200, "should work with global whitelist for pau1");

        vm.prank(CBEAM1);
        bytes memory ret2 = configurator.callControllerAction(address(pau2), data);
        assertEq(abi.decode(ret2, (uint256)), 200, "should work with global whitelist for pau2");
    }
    ...
    function testCallControllerActionArbitraryData() public {
        ...
        assertEq(pau1.lastCallData(), data, "arbitrary data should be passed correctly");
    }
    ...
    function testFullRateLimitWorkflow() public {
        ...
        _setupRateLimitData(pau1, key, 800 * WAD, 8 * WAD, 800 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 3_600);
        beamState.setMaxChange(address(pau1), 2 * WAD); // 2x

        vm.warp(block.timestamp + 3_600); // Warp to allow first unsafe increase

        // 1. Decrease limits (safe, immediate)
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 700 * WAD, 7 * WAD);
        ...
        data = pau1.getRateLimitData(key);
        assertEq(data.maxAmount, 1_200 * WAD, "step 2: first increase should work");
        uint256 firstIncrease = configurator.zzz(address(pau1), key);
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 2_400 * WAD, 24 * WAD); // Exactly 2x
        ...
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/maxChange-maxAmount");
        configurator.setRateLimit(address(pau1), key, 4_900 * WAD, 24 * WAD); // >2x (2.04x)
    }

    function testMultipleCBeamsMultiplePaus() public {
        bytes32 key = keccak256("multi-key");

        // Setup CBEAM1 for PAU1
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key, 500 * WAD, 5 * WAD, 500 * WAD, block.timestamp);

        // Setup CBEAM2 for PAU2
        _setupCBeam(address(pau2), CBEAM2);
        _setupDefaultRateLimits(key, address(pau2), 2_000 * WAD, 20 * WAD);
        _setupRateLimitData(pau2, key, 1_000 * WAD, 10 * WAD, 1_000 * WAD, block.timestamp);

        // CBEAM1 configures PAU1
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 600 * WAD, 6 * WAD);

        // CBEAM2 configures PAU2
        vm.prank(CBEAM2);
        configurator.setRateLimit(address(pau2), key, 1_200 * WAD, 12 * WAD);

        // Verify independent operation
        RateLimitsLike.RateLimitData memory data1 = pau1.getRateLimitData(key);
        RateLimitsLike.RateLimitData memory data2 = pau2.getRateLimitData(key);

        assertEq(data1.maxAmount, 600 * WAD, "PAU1 should have its own limits");
        assertEq(data2.maxAmount, 1_200 * WAD, "PAU2 should have its own limits");

        // CBEAM1 cannot configure PAU2
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/not-authorized-cBeam");
        configurator.setRateLimit(address(pau2), key, 1_500 * WAD, 15 * WAD);
    }
    ...
    function testIncreaseThenDecreaseThenIncrease() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 1_100 * WAD, 11 * WAD);
        uint256 zzzAfterIncrease = configurator.zzz(address(pau1), key);
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 900 * WAD, 9 * WAD);
        assertEq(configurator.zzz(address(pau1), key), zzzAfterIncrease, "zzz should not change on decrease");
    ...
    function testEdgeCaseZeroValues() public {
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key, 0, 0);
    ...
    function testDifferentKeysIndependentZzz() public {
        ...
        _setupCBeam(address(pau1), CBEAM1);
        _setupDefaultRateLimits(key1, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupDefaultRateLimits(key2, address(pau1), 1_000 * WAD, 10 * WAD);
        _setupRateLimitData(pau1, key1, 900 * WAD, 9 * WAD, 900 * WAD, block.timestamp);
        _setupRateLimitData(pau1, key2, 900 * WAD, 9 * WAD, 900 * WAD, block.timestamp);
        beamState.setHop(address(pau1), 3_600);
        ...
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key1, 1_100 * WAD, 11 * WAD);

        // Increase key2 immediately should work (different key)
        vm.prank(CBEAM1);
        configurator.setRateLimit(address(pau1), key2, 1_100 * WAD, 11 * WAD);

        // But can't increase key1 again immediately
        vm.prank(CBEAM1);
        vm.expectRevert("Configurator/increment-too-soon");
        configurator.setRateLimit(address(pau1), key1, 1_200 * WAD, 12 * WAD);

        assertGt(configurator.zzz(address(pau1), key1), 0, "key1 should have zzz set");
        assertGt(configurator.zzz(address(pau1), key2), 0, "key2 should have zzz set");
    }
}
